...
export class AdminApiGuard implements CanActivate {
  ...
  constructor(config: ConfigService) {
    this.expected = config.get<string>('ACCOUNTING_ADMIN_API_KEY');
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (!this.expected || provided !== this.expected) {
      throw new UnauthorizedException('A valid x-admin-api-key header is required');
    }
    return true;
  }
}
