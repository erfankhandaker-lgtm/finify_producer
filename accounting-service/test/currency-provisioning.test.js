...
test('new currency provisioning creates all seven wallets before its configuration', async () => {
  ...
  const manager = {
    query: async (sql, values = []) => {
      ...
      if (sql.includes('FROM public."SW_TBL_WALLET_TYPE"')) {
        return [105, 110, 113, 114, 115, 116, 117].map(Wallet_ID => ({ Wallet_ID }));
      }
      ...
      if (sql.includes('INSERT INTO public.sw_tbl_accounting_configuration')) {
        assert.deepEqual(createdCodes, [105, 110, 113, 114, 115, 116, 117]);
  ...
  assert.equal(result.wallets.length, 7);
  assert.deepEqual(result.wallets.map(wallet => wallet.walletCode), [105, 110, 113, 114, 115, 116, 117]);
});
