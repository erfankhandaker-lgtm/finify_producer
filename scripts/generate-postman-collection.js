...
const captureOnboardingProgress = [
  'if (pm.response.code >= 200 && pm.response.code < 300) {',
  '  const json = pm.response.json();',
  '  const data = json.payload || json;',
  ...
  '  if (data.currentNodeKey) pm.collectionVariables.set("onboardingCurrentNodeKey", data.currentNodeKey);',
  '  if (data.currentNodeType) pm.collectionVariables.set("onboardingCurrentNodeType", data.currentNodeType);',
  '}',
];
...
const producerItems = [
  ...
  folder('Customer onboarding journey', [
    producerRequest('Start or resume onboarding journey', 'POST', '/api/v1/onboarding/instances', {
      auth: noAuth,
      headers: [
        { key: 'X-Correlation-ID', value: 'onboarding-{{$guid}}', type: 'text' },
      ],
      body: {
        tenantId: '{{onboardingTenantId}}',
        countryCode: '{{onboardingCountryCode}}',
        channelCode: '{{onboardingChannelCode}}',
        customerType: '{{onboardingCustomerType}}',
        phoneNumber: '{{onboardingPhoneNumber}}',
      },
      tests: captureOnboardingProgress,
      description: 'Pre-auth route. Selects the active tenant/country/channel journey and securely creates or resumes its durable instance.',
    }),
    producerRequest(
      'Resume onboarding journey',
      'GET',
      '/api/v1/onboarding/instances/resume?token={{onboardingResumeToken}}',
      {
        auth: noAuth,
        headers: [
          { key: 'X-Correlation-ID', value: 'onboarding-{{$guid}}', type: 'text' },
        ...
        tests: captureOnboardingProgress,
        description: 'Returns current durable progress. Treat the resume token as a bearer-like secret and suppress query-string logging.',
      },
    ),
    producerRequest(
      'Advance generic onboarding step',
      'POST',
      '/api/v1/onboarding/instances/{{onboardingInstanceId}}/steps',
      {
        auth: noAuth,
        headers: [
          { key: 'X-Correlation-ID', value: 'onboarding-{{$guid}}', type: 'text' },
          { key: 'X-Onboarding-Resume-Token', value: '{{onboardingResumeToken}}', type: 'text' },
          { key: 'Idempotency-Key', value: 'onboarding-{{$guid}}', type: 'text' },
        ],
        body: {
          nodeKey: '{{onboardingCurrentNodeKey}}',
          outcome: 'SUCCESS',
          output: {},
        },
        tests: captureOnboardingProgress,
        description: 'Only START and PHONE_CAPTURE may use this generic endpoint. OTP, PIN, consent, KYC, wallet and credit nodes require their dedicated verified services.',
      },
    ),
  ], 'Customer-app onboarding runtime. The focused executable collection is generated by npm run postman:onboarding.'),
...
const collection = {
  ...
  variable: [
    ...
    { key: 'onboardingTenantId', value: '', type: 'string' },
    { key: 'onboardingCountryCode', value: 'UGA', type: 'string' },
    { key: 'onboardingChannelCode', value: 'MOBILE_APP', type: 'string' },
    { key: 'onboardingCustomerType', value: 'INDIVIDUAL', type: 'string' },
    { key: 'onboardingPhoneNumber', value: '+256700000001', type: 'string' },
    ...
    { key: 'onboardingCurrentNodeKey', value: '', type: 'string' },
    { key: 'onboardingCurrentNodeType', value: '', type: 'string' },
